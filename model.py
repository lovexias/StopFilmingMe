        ...
        while frame_idx < total_frames:
            ...
            if frame_idx % yolo_skip_frames == 0:
                ...
                for detected_bbox in last_detected_people:
                    result = match_person_to_blur_list(detected_bbox, [{"bbox": last_matched_bbox, "center": ((last_matched_bbox[0]+last_matched_bbox[2])//2,
                                                                                                            (last_matched_bbox[1]+last_matched_bbox[3])//2)}])
                    if result:
                        matched = detected_bbox
                        break
                if matched:
                    last_matched_bbox = matched
